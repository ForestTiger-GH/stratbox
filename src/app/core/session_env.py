"""App-facing state surfaces inside an AppDock-managed session."""
...
class RuntimeStateRecord:
    ...
    def last_scenario_ok(self) -> bool | None:
        value = self.surface_state.get('last_scenario_ok')
    ...
    def last_scenario_log(self) -> str | None:
        value = self.surface_state.get('last_scenario_log')
        return str(value) if value not in (None, '') else None
