...
from app.presentation.desktop.dialogs.diagnostics_dialog import DiagnosticsDialog
from app.presentation.desktop.dialogs.settings_dialog import SettingsDialog
...
class MainWindow(QMainWindow):
    ...
    def _connect_runtime(self) -> None:
        ...
        self.runtime.scenario_coordinator.case_created.connect(lambda *_: self._refresh_context_views())
        self.runtime.scenario_coordinator.case_updated.connect(lambda *_: self._refresh_context_views())
        self.runtime.scenario_coordinator.event_appended.connect(lambda *_: self._refresh_context_views())
        self.runtime.scenario_coordinator.artifacts_created.connect(lambda *_: self._refresh_context_views())
        self.runtime.scenario_coordinator.log_created.connect(lambda *_: self._refresh_context_views())
    ...
    def _toggle_background_process(self, process_id: str, enabled: bool) -> None:
        ...
        self.runtime.event_store.add(OperationalEvent.create(
            ...
            body=f'{process_id}: {"включён" if enabled else "выключен"}',
    ...
    def _refresh_context_views(self) -> None:
        ...
        self.body.left_panel.people_panel.refresh()
