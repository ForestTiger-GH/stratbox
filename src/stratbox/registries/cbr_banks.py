...
def _load_legacy_regn_sort_map() -> dict[str, int]:
    """
    Возвращает словарь regn -> sort (int) из cbr_legacy.csv.

    Интеграция legacy делается строго по регистрационному номеру (regn),
    чтобы избежать дублей по названиям и проблем с действующей/не действующей лицензией.

    Требования к cbr_legacy.csv:
    - колонки: regn, sort
    """
    ...
    need = {"regn", "sort"}
    if not need.issubset(cols.keys()):
        raise ValueError("cbr_legacy.csv must have columns: regn, sort")

    c_regn = cols["regn"]
    c_sort = cols["sort"]

    tmp = df[[c_regn, c_sort]].copy()
    ...
    # при дублях по regn берём минимальный sort
    for _, row in tmp.iterrows():
        regn = str(row[c_regn]).strip()
        if not regn or regn.upper() == "NAN":
            continue
        sort_val = int(row[c_sort])
...
def read() -> pd.DataFrame:
    ...
    # 3) legacy: интеграция строго по regn
    regn_to_sort = _load_legacy_regn_sort_map()

    legacy_sort = out["regn"].astype(str).str.replace(".0", "", regex=False).str.strip().map(regn_to_sort)

    # В is_legacy: либо int (порядок legacy), либо False
    out["is_legacy"] = legacy_sort.astype(object).where(pd.notna(legacy_sort), False)
